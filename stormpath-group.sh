...
for groupHref in "${groups[@]}"; do
  ...
  resp=$(/usr/bin/curl -vu "$apiKeyId":"$apiKeySecret" -H 'Accept: application/json' -H 'Content-Type: application/json' -G --data-urlencode "$userfield=$user" "$groupHref/accounts"  | /usr/local/bin/jq '.size')
